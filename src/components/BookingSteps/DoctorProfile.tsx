...
export const DoctorProfile = ({ doctor }: DoctorPrfileInterface) => {
  return (
    <Card className="sticky top-8 shadow-lg border-0">
      <CardContent className="p-8">
        <div className="mb-6 text-center">
          
          <Avatar className="size-32 mx-auto ring-4 ring-gray-100">
            <AvatarImage
              src={doctor?.profileImage}
              alt={doctor?.name}
            ></AvatarImage>
            <AvatarFallback className="bg-linear-to-br fill-blue-500 to-blue-600 text-2xl font-bold">
              {doctor?.name?.charAt(0)?.toUpperCase()}
            </AvatarFallback>
          </Avatar>

          <h2 className="text-2xl font-bold to-gray-900 mb-2">{doctor.name}</h2>
          ...
          <p className="text-gray-500 text-sm mb-4">
            {doctor.experience > 1 ? "year experience" : "years experience"}
          </p>
          ...
          <div className="justify-center flex flex-wrap gap-2 mb-6">
            {doctor.isVerified && (
              <Badge
                variant={"secondary"}
                className="bg-green-100 text-green-800"
              >
                <Award className="w-3 h-3 mr-1">Verified</Award>
              </Badge>
            )}
        ...
        <div className="space-x-4">
          ...
          <div className="flex items-center justify-between p-4 bg-linear-to-br from-green-50 to-gray-100 rounded-lg">
            <div>
