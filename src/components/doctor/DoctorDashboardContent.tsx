...
import PrescriptionModal from "./PrescriptionModelProps";
...
const DoctorDashboardContent = () => {
  ...
  const handleSavePrescription = async (
    ...
    try {
      ...
      console.error("Failed to complete consultaion", error);
    } finally {
  ...
  const formateDate = (dateString: string) => {
    return new Date(dateString).toLocaleDateString("end-US", {
      hour: "2-digit",
      minute: "2-digit",
      hour12: true,
    });
  ...
  const canJoinCall = (appointment: any) => {
    const appointentTime = new Date(appointment.slotStartIso);
    const now = new Date();
    const diffMintues =
      (appointentTime.getTime() - now.getTime()) / (1000 * 60);

    return (
      diffMintues <= 15 && // not earlier than 15 min before start
  ...
  if (loading || !dashboardData) {
    return (
      <>
        ...
        <div
          className={
            "min-h-screen bg-linear-to-br from-blue-50 via-white to-indigo-50 pt-16"
          }
        >
          <div className="container mx-auto px-4 py-8">
            <div className="animate-pulse space-y-8">
              ...
              <div className="grid grid-cols-4 gap-6">
                {[...Array(4)].map((_, i) => (
                  <div key={i} className="h-32 bg-gray-300 rounded-md"></div>
                ))}
              </div>
            </div>
          </div>
